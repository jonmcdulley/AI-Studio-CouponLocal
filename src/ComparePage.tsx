...
const COMPARE_DATA: Record<string, Deal[]> = {
  Travel: [
    ...
    { store: "KKday Global",  offer: "Tours and Activities Worldwide",  description: "Book tours, day trips and experiences across Asia and beyond.",        logo: logo("kkday.com"),        affiliateUrl: "https://invl.me/clnfk9w"   },
    ...
    { store: "Jetpac Global", offer: "Global eSIM and Travel Insurance",description: "Stay connected anywhere with eSIM data plans and travel insurance.",   logo: logo("jetpacglobal.com"), affiliateUrl: "https://invl.me/clng029"   },
  ],
  Fashion: [
    ...
    { store: "Zalora PH",       offer: "25% Off First App Order",         description: "New customers get 25% off first in-app purchase (min. spend P2195).",    logo: logo("zalora.com.ph"),    affiliateUrl: "https://invl.me/clng084"     },
    { store: "Shopee PH",       offer: "Daily Deals and Flash Sales",     description: "Millions of products with daily vouchers and flash sales.",               logo: logo("shopee.ph"),        affiliateUrl: "https://invl.me/clng080"     },
    ...
    { store: "Charles & Keith", offer: "New Arrivals - Bags and Shoes",   description: "Shop the latest bags, shoes and accessories from Charles & Keith PH.",   logo: logo("charleskeith.com"), affiliateUrl: "https://invl.me/clnfk9t"     },
  ],
  Beauty: [
    { store: "Sephora PH", offer: "Top Beauty Brands and Skincare", description: "Shop premium beauty, skincare and makeup from the world's top brands.", logo: logo("sephora.ph"),    affiliateUrl: "https://invl.me/clnfka2"     },
    { store: "Papique",    offer: "Premium Beauty and Skincare",    description: "Curated beauty and skincare products delivered to your door.",           logo: logo("papique.com"),   affiliateUrl: "https://invl.us/clnfqhf"     },
    { store: "Stylevana",  offer: "K-Beauty and J-Beauty Deals",    description: "Shop Korean and Japanese beauty brands at great prices.",               logo: logo("stylevana.com"), affiliateUrl: "https://miniurl.app/clng02d" },
  ],
};
...
function DealCard({ deal, rank }: { deal: Deal; rank?: number }) {
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 hover:shadow-md transition-shadow flex flex-col justify-between">
      ...
      <a href={deal.affiliateUrl} target="_blank" rel="noopener noreferrer" className="block text-center bg-indigo-600 hover:bg-indigo-700 text-white font-semibold py-2.5 rounded-xl text-sm transition-colors">
        Get Deal
      </a>
    </div>
  );
    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-white to-purple-50 p-4 pb-24 md:p-8">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="relative mb-8">
          ...
          />
          {search && (
            <button onClick={() => setSearch("")} className="absolute right-4 top-4 text-gray-400 hover:text-gray-600 text-xl">
              x
            </button>
          )}
        </div>
        {search.trim() ? (
          <div className="mb-10">
            <h2 className="text-lg font-semibold text-gray-700 mb-4">
              {searchResults.length > 0 ? searchResults.length + " results for " + search : "No results for " + search}
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {searchResults.map((deal, i) => (<DealCard key={i} deal={deal} />))}
            </div>
          </div>
        ) : (
          <>
            <div className="flex gap-3 mb-6 flex-wrap">
              {Object.keys(COMPARE_DATA).map((cat) => (
                <button
                  ...
                  className={"px-5 py-2 rounded-full font-semibold text-sm transition-all " + (activeCategory === cat ? "bg-indigo-600 text-white shadow-md" : "bg-white text-gray-600 border border-gray-200 hover:border-indigo-300")}
                >
                  {categoryIcons[cat]}
                </button>
              ))}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {COMPARE_DATA[activeCategory].map((deal, i) => (<DealCard key={i} deal={deal} rank={i + 1} />))}
            </div>
          </>
        )}
      </div>
